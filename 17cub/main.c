...
int parse_map_line(char *line, t_cub_data *data) {
    int len = strlen(line);
    char last_char = ' ';  // Initialize to a space for logic simplicity

    // Check every character in the line
    for (int i = 0; i < len; i++) {
        char c = line[i];
        if (c != '0' && c != '1' && c != 'N' && c != 'S' && c != 'E' && c != 'W' && c != ' ' && c != '\t') {
            printf("Error: Invalid character '%c' found in the map.\n", c);
            return -1;
        }
        if (c == '1') {
            last_char = c;
        } else if ((c == ' ' || c == '\t') && last_char == '1') {
            // Check the rest of the line after this space to see if there are non-space characters
    ...
    // Ensure the line starts and ends with a '1'
    if (line[0] != '1' || last_char != '1') {
        printf("Error: Line must start and end with a wall '1'.\n");
        return -1;
    }

    // Allocate and store the line in the map data structure
...
void print_map_shape(t_cub_data *data) {
    printf("Map Shape:\n");
    for (int i = 0; i < data->map_height; i++) {
        int line_length = (int)strlen(data->map[i]);  // Cast to int
...
int parse_cub3(int argc, char **argv, t_cub_data *data) {
    ...
    if (validate_map(data)) {  // Corrected: use data directly
        printf("The .cub file has been successfully parsed and validated.\n");
        print_map_shape(data);  // Corrected: use data directly
    } else {
        printf("Map validation failed\n");
    }
...
int main(int argc, char **argv) {
    ...
    init_cub3(&data);  // Initialize data structure

    parse_result = parse_cub3(argc, argv, &data);  // Corrected: pass address of data
    if (parse_result != 0) {
        return parse_result;  // Handle error
    }
